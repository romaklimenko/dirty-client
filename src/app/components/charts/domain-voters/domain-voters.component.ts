...
@Component({
  selector: 'app-domain-voters',
  templateUrl: './domain-voters.component.html',
  styleUrls: ['./domain-voters.component.css']
})
export class DomainVotersComponent implements OnInit, AfterViewInit {
  ...
  render(): void {
    ...
    const key = d => {
      let virtual = '';
      if (d.voter in sockpuppets) {
        // tslint:disable-next-line: quotemark
        virtual = ` (виртуал ${sockpuppets[d.voter].join(", ")})`;
      }
    ...
    this.voters.top_lovers.forEach(render);
    this.voters.top_haters.forEach(render);
